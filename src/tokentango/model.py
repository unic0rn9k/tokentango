...
class BertClassifier(nn.Module):
    def __init__(self, max_seq_len, vocab_size, device):
        ...
        self.transformer_encoder = nn.TransformerEncoder(
            self.encoder_layer, num_layers=6, norm=nn.LayerNorm(EMBEDDING_DIM)
        )
    ...
    def embed(self, seq):
        ...
        expanded_positional_encoding = self.positional_encoding.unsqueeze(
            0
    ...
    def classify_loss(self, hidden, mb_y):
        ...
        loss_cls = torch.nn.functional.huber_loss(logits.view(-1), mb_y)
        return loss_cls

    def mlm_loss(self, hidden, mb_x):
        outputs = self.distribution(hidden)[:, 1:, :]
        loss_mlm = torch.nn.functional.cross_entropy(
            outputs.reshape(-1, self.vocab_size), mb_x[:, 1:].reshape(-1)
        )
        return loss_mlm
